...
export const createAgendamento = async (agendamento) => {
  // Django espera 'servico' como FK ID (não 'servico_id')
  const payload = { ...agendamento };
  if (payload.servico_id !== undefined) {
    payload.servico = payload.servico_id;
...
export const updateAgendamento = async (id, agendamento) => {
  ...
  const response = await axios.put(`${API}/agendamentos/${id}/`, payload);
  return response.data;
};
